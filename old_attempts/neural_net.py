...
class NeuralNet(object):
    def __init__(self, training_data=None):
        self.synapses = {}
        if training_data:
            self.corpus = read_data(training_data)
            self.corpus.set_common_words(qty=200)
            self.corpus.set_vector_mapping(qty=200)
        self.training_input_data = []
        self.training_output_data = []

    def think(self, doc):
        synapse_0 = self.synapses[0]
        synapse_1 = self.synapses[1]
        # Layer 0
        L0 = np.array(doc.clean(qty=200))
        # Layer 1 is mult of input and hidden layer
    ...
    def set_training_data(self):
        for doc in self.corpus.documents:
            sample = doc.clean(qty=200)
            sample_arch = [0]*12
            ind = ARCHS.index(doc.arch)
            sample_arch[ind] = 1
            self.training_input_data.append(sample)
            self.training_output_data.append(sample_arch)

    def train(self, hidden_neurons=10, alpha=1, epochs=1000):
        ...
        self.synapses[0] = 2*np.random.random((len(X[0]), hidden_neurons)) - 1
        self.synapses[1] = 2*np.random.random((hidden_neurons, len(ARCHS))) - 1

        for j in xrange(epochs):
            if j%100 == 0:
                print j
            # Feed forward
            # through layers 0, 1, 2
            L0 = np.array(X)
            L1 = sigmoid(np.dot(L0, self.synapses[0]))
            L2 = sigmoid(np.dot(L1, self.synapses[1]))
            ...
            L1_error = np.dot(L2_delta, self.synapses[1].T)
            ...
            # Show error
            errors = [sum_square_diff(L2[i], Y[i]) for i in range(len(L2))]
            print sum(errors)

            self.synapses[1] += alpha * synapse_1_update
            self.synapses[0] += alpha * synapse_0_update
...
if __name__ == '__main__':
    print "Initializing..."
    N = NeuralNet(training_data='big_train.csv')
    N.set_training_data()
    print "Done"

    print "Training..."
    S = N.train(hidden_neurons=20, alpha=0.01, epochs=10000)
    print "Done"

    synapse = {'synapse0': S[0].tolist(), \
            'synapse1': S[1].tolist(),}

    with open('synapses.json','w') as outfile:
        json.dump(synapse, outfile)
